# -*- coding: utf-8 -*-
"""mini42_probe.py — v13.65 재확인(T11·C6·S4) + 추천 후보 원칙 확인(M2 일반 추천, M2b 안정형 추천). 5문항.
사용: python mini42_probe.py && python claim_check.py mini42_out.txt"""
...
Q = [
    ("T11", "30대 직장인인데 공격적으로 굴릴 연금펀드 하나 추천해줘",
     ...
     + ([] if re.search(r"위험등급\s*[1-6]\s*등급", a) or "확정하지 못했습니다" in a else ["후보_등급표기_없음(원문 확인)"])),
    ("C6", "저 연금 얼마 받을 수 있어요?",
     lambda a: ([] if re.search(r"계산할 수 없습니다", a[:200]) else ["★한계고지_누락"])
     + ([] if not re.search(r"사용자부담금|수급기간|연금대기자|(?m)^\s*\d+[.)]\s", a) else ["★특정계좌_요건목록_잔존"])),
...
def main():
    ft = open("mini42_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("5문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini42_out.txt (판정은 후보, 원문 확인 필요)")
