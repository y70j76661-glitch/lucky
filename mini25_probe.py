# -*- coding: utf-8 -*-
"""mini25_probe.py — v13.47(등급 오출력·절세 단정·중복 정리) 확인 5문항. 사용: python mini25_probe.py && python claim_check.py mini25_out.txt"""
...
def _blocks_ok(a):
    ...
    if "자료에서 확인 필요" in body and "근거 문서:" in body:
        f.append("등급자리표시_잔존")
    if re.search(r"(?:이자소득세|기타소득세|세금)[^.!?\n]{0,15}피할\s*수", body):
        f.append("절세단정_잔존")
    if re.search(r"(?m)^\s*[-•]?\s*이러한 상품은 (?:후보로|실적배당형)", body):
        f.append("주어없는_표준문장_잔존")
    return f
...
def main():
    ft = open("mini25_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("5문항 모두 OK → v13.47 확인 통과" if not bad else f"확인 필요 {bad}문항 → mini25_out.txt")
