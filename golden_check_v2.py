# -*- coding: utf-8 -*-
"""golden_check.py(v2) — 기준 버전 고정 회귀 비교(세트 B 요구 키워드 추가). 사용: python golden_check.py <새 결과> <기준 결과>
...
REQ = {  # 질문 ID → 답변에 반드시 있어야 하는 요구 키워드(정규식)
    ...
    "B6": r"확인되지 않|확인할 수 없|확인해 드리지 못", "B7": r"4\.63", "B10": r"900", "B11": r"16\.5",
...
BAD = re.compile(r"추천\s*(?:드립니다|합니다|드리겠습니다)|수상|최우수|1위|높은\s*성장\s*가능성을\s*제공|회복\s*가능성이\s*큽|극복할\s*가능성|안전합니다|적합합니다|"
                 r"원금(?:이|을)?\s*보장(?:됩니다|되는|하는)|세금이\s*없습니다|(?<!\*)\*(?!\*)|은\(는\)|연금계좌\s+계좌|차후 점에|있하지만")
