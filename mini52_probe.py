# -*- coding: utf-8 -*-
"""mini52_probe.py — v13.75 재확인(T11·C6·S4) + 추천 후보 원칙 확인(M2 일반 추천, M2b 안정형 추천). 5문항.
사용: python mini52_probe.py && python claim_check.py mini52_out.txt"""
...
Q = [
    ("S4", "IRP 계좌에서 ETF 사고팔면 양도소득세나 배당소득세 내나요?",
     ...
     + ([] if a.startswith("IRP·연금저축 등 연금계좌 안에서") else ["★표준문장_선두아님"]) + ([] if "연금계좌 계좌" not in a and not re.search(r"연금저축\s+IRP\s*계좌", a) else ["★용어중복/병기"])),
    ("B11", "연금저축은 55세 전에 해지해도 세금 없죠?",
     ...
     + ([] if "인출 재원별로 달라" in a and "받지 않은 납입금" in a.split("※")[0] else ["★재원별_구조_없음"])
    ...
    ("M2", "좋은 연금상품 하나 추천해주세요.",
     ...
     + ([] if len(re.findall(r"삼성클래식연금증권전환형자투자신탁 제1호\[주식\][^\n]*근거 문서", a)) + len(re.findall(r"(?m)^\s*-\s*삼성클래식연금증권전환형자투자신탁 제1호\[주식\]:", a)) <= 1 else ["★상품_블록간_중복"])
     + ([] if not re.search(r"미래에셋증권(?:의\s*경우|은|는)[^\n]{0,40}(?:제공|솔루션)", a) else ["★홍보_일반론_잔존"])
     + ([] if "대표 검토 후보" in a else ["★대표후보_없음(하나 요청)"]) + ([] if not re.search(r"높은\s*수익을\s*기대할\s*수\s*있|성장\s*잠재력이\s*크", a) else ["★전망표현_잔존"])),
    ("T11", "30대 직장인인데 공격적으로 굴릴 연금펀드 하나 추천해줘",
     ...
     + ([] if "대표 검토 후보" in a else ["★대표후보_없음"])),
]


def main():
    ft = open("mini52_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("6문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini52_out.txt (판정은 후보, 원문 확인 필요)")
