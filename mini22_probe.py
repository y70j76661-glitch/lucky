# -*- coding: utf-8 -*-
"""mini22_probe.py — v13.44(등급 근거 기반 표준 문장) 확인 5문항. 사용: python mini22_probe.py && python claim_check.py mini22_out.txt"""
...
def main():
    ft = open("mini22_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("5문항 모두 OK → v13.44 확인 통과" if not bad else f"확인 필요 {bad}문항 → mini22_out.txt")
