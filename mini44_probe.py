# -*- coding: utf-8 -*-
"""mini44_probe.py — v13.67 재확인(T11·C6·S4) + 추천 후보 원칙 확인(M2 일반 추천, M2b 안정형 추천). 5문항.
사용: python mini44_probe.py && python claim_check.py mini44_out.txt"""
...
Q = [
    ...
    ("S4", "IRP 계좌에서 ETF 사고팔면 양도소득세나 배당소득세 내나요?",
     ...
     + ([] if "통산" not in a else ["통산_잔존(근거 확인)"]) + ([] if a.count("즉시 과세되지 않고(과세이연)") <= 1 else ["★표준문장_중복"])
     + ([] if not re.search(r"(?:양도소득세|배당소득세)[^\n]{0,20}(?:부과되지 않|내지 않)", a) else ["★면제단정_잔존"])),
    ("M2", "좋은 연금상품 하나 추천해주세요.",
     lambda a: ([] if not re.search(r"추천\s*(?:드립니다|합니다|드리겠습니다)", a) else ["★단정추천_잔존"])
     + ([] if not re.search(r"근거 문서:[^\n]*확인되지 않아", a) else ["★등급미확인_후보_잔존"])
     + ([] if re.search(r"보장되지 않", a) or "예금" in a else ["비보장고지_누락"]) + ([] if not re.search(r"꾸준한\s*수익|극복할\s*가능성", a) else ["★보장성_표현_잔존"])),
    ("M2b", "은퇴가 10년 남았는데 안정적인 연금펀드 추천해줘.",
     ...
     + ([] if re.search(r"[1-6]\s*등급|확정하지 못", a) else ["등급표기_없음(원문 확인)"]) + ([] if "은(는)" not in a else ["조사_미정합"])),
]


def main():
    ft = open("mini44_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("5문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini44_out.txt (판정은 후보, 원문 확인 필요)")
