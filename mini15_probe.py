# -*- coding: utf-8 -*-
"""mini15_probe.py — v13.33 확인 4문항. 전체 회귀 아님.
사용: cd /root/app && source venv/bin/activate && python mini15_probe.py && python3 claim_check.py mini15_out.txt"""
...
Q = [
    ("M2", "좋은 연금상품 하나 추천해주세요.",
     lambda a: ([] if not re.search(BAD_M2, a.split("[참고 문서]")[0]) else ["★표현_잔존:" + (re.search(BAD_M2, a.split("[참고 문서]")[0]).group(0))]) + ([] if not re.search(r"(?m)^\s*\d+\.\s*\**\s*$|^\s*\d+\.\s*\*이 상품|^\s*\d+\.[^\n]*[:：]\s*\n\s*\n", a) else ["★목록_구조_깨짐/빈항목"]) + ([] if len(re.findall(r"보장되지\s*않", a.split("[참고 문서]")[0])) >= 2 else ["상품별_비보장고지_부족"])),
    ("M2b", "은퇴가 10년 남았는데 안정적인 연금펀드 추천해줘.",
     lambda a: ([] if not re.search(BAD_M2, a.split("[참고 문서]")[0]) else ["★표현_잔존:" + (re.search(BAD_M2, a.split("[참고 문서]")[0]).group(0))]) + ([] if "보장되지 않습니다" in a or "보장은 아님" in a else ["비보장_고지_없음"]) + ([] if (not re.search(r"높은\s*위험|높은위험", a) or "위험 고지" in a) else ["★높은위험_상품에_위험고지_없음"]) + ([] if len(re.findall(r"보장되지\s*않", a.split("[참고 문서]")[0])) >= len(re.findall(r"(?m)^\s*\d+\.\s", a.split("[참고 문서]")[0])) else ["★상품별_비보장고지_부족"])),
...
def main():
    ft = open("mini15_out.txt", "w", encoding="utf-8")
    bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
    ...
    ft.close()
    print("=" * 50)
    print("4문항 모두 OK → v13.33 확인 통과" if not bad else f"확인 필요 {bad}문항 → mini15_out.txt 원문 확인")
