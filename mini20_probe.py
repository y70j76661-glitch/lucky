# -*- coding: utf-8 -*-
"""mini20_probe.py — v13.42(추천 렌더러) 확인 5문항. 사용: python mini20_probe.py && python claim_check.py mini20_out.txt"""
...
def main():
    ft = open("mini20_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("5문항 모두 OK → v13.42 확인 통과" if not bad else f"확인 필요 {bad}문항 → mini20_out.txt")
