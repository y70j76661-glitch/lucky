# -*- coding: utf-8 -*-
"""mini58_probe.py — 세트 B 회귀(v13.60 이후 재실행 없던 문항 12개). 판정식은 원 프로브(mini36·mini39·mini28)와 동일.
...
사용: python mini58_probe.py && python golden_check.py mini58_out.txt base_setB.txt && python claim_check.py mini58_out.txt"""
...
def main():
    ft = open("mini58_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
    ...
    ft.close(); print("=" * 50); print("12문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini58_out.txt (판정은 후보, golden_check 결과가 우선)")
