# -*- coding: utf-8 -*-
"""mini55_probe.py — v13.76 재확인(T11·C6·S4) + 추천 후보 원칙 확인(M2 일반 추천, M2b 안정형 추천). 5문항.
사용: python mini55_probe.py && python claim_check.py mini55_out.txt"""
...
def main():
    ft = open("mini55_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("11문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini55_out.txt (판정은 후보, 원문 확인 필요)")
