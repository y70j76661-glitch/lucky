# -*- coding: utf-8 -*-
"""mini50_probe.py — v13.73 재확인(T11·C6·S4) + 추천 후보 원칙 확인(M2 일반 추천, M2b 안정형 추천). 5문항.
사용: python mini50_probe.py && python claim_check.py mini50_out.txt"""
...
Q = [
    ...
    ("M2", "좋은 연금상품 하나 추천해주세요.",
     ...
     + ([] if not re.search(r"꾸준한\s*수익|극복할\s*가능성|안전한\s*투자를\s*지향|손실의?\s*가능성이\s*낮(?:습니다|지만)|분들?께\s*적합|고수익을\s*기대하는|높은\s*수익을\s*목표로\s*합니다|없지는\s*않으며\.|안정적인\s*수익을\s*원하는", a) else ["★단정/보장성_표현_잔존"])
...
def main():
    ft = open("mini50_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
            a, tr = _ask(qid, q)
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("6문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini50_out.txt (판정은 후보, 원문 확인 필요)")
