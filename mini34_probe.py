# -*- coding: utf-8 -*-
"""mini34_probe.py — v13.56 재확인 3문항(T6·T11·M2).
사용: python mini34_probe.py && python claim_check.py mini34_out.txt && python cite_check.py mini34_out.txt"""
...
def _entity_mix(a):
    ...
    for blk in re.split(r"\n\s*\n", a.split("[참고 문서]")[0]):
        ...
        if not m:
            continue
        if re.search(r"경우|성향|선호|중시|원하", m.group(1)):
            continue
...
Q = [
    ("T6", "미래에셋 TDF2045랑 삼성클래식연금 주식형 중에 위험등급이 더 낮은 건 뭐예요?",
     lambda a: ([] if re.search(r"[1-6]\s*등급", a) else ["등급_누락"]) + ([] if "대조 불가" not in a else ["대조불가_고지"]) + ([] if re.search(r"삼성클래식[^\n]{0,40}2\s*등급|2\s*등급[^\n]{0,40}삼성클래식|주식[^\n]{0,30}2\s*등급", a) else ["삼성주식2등급_누락"]) + ([] if "0.47" not in a else ["★창작수치0.47"]) + ([] if not (re.search(r"2\s*등급[^\n]{0,60}보다\s*(?:더\s*)?높", a) and len(re.findall(r"2\s*등급", a.split("핵심 차이")[0])) >= 2) else ["★동일등급_우열단정"]) + ([] if not re.search(r"등급\)\s*/\s*\(", a) else ["★상충셀_잔존"]) + ([] if not re.search(r"위험\s*등급이\s*(?:더|가장)\s*(?:낮|높)습니다", a) else ["★우열단정_잔존"]) + ([] if not re.search(r"TDF2045[^\n]{0,80}\d\s*등급", a.split("핵심 차이")[-1].split("[위험등급")[0]) else ["★TDF등급_요약단정"]) + ([] if re.search(r"등급 비교\(근거 문서 기준\)|검증 불가|위험등급이 같으므로", a) or "보다" not in a else ["우열_미확정문장"])),
    ("T11", "30대 직장인인데 공격적으로 굴릴 연금펀드 하나 추천해줘",
     lambda a: ([] if "근거 문서:" in a else ["근거문서줄_없음"]) + ([] if "(출처:아래" not in a and "부문 우수상" not in a else ["★원문덤프"]) + ([] if re.search(r"보장되지\s*않", a) else ["★비보장고지_누락"]) + ([] if "우선순위" in a or not re.search(r"(?m)^\s*2[.)]\s", a) else ["무순위고지_누락"]) + ([] if not re.match(r"^\s*-\s", a) else ["★머리말없는_항목"]) + ([] if not re.search(r"적합하다고\s*판단|추천드릴\s*수\s*있습니다|추천드립니다", a) else ["단정추천_잔존"]) + ([] if not re.search(r"수상|인정받", a) else ["판촉주장_잔존"]) + ([] if not re.search(r"상품 예시:\s*TIGER[^\n]*은\(는\)\s*$", a, re.M) else ["★상품예시_꼬리"]) + (["★엔티티혼입"] if _entity_mix(a) else [])),
    ("M2", "좋은 연금상품 하나 추천해주세요.",
     lambda a: ([] if "근거 문서:" in a else ["근거문서줄_없음"]) + ([] if "(출처:아래" not in a and "부문 우수상" not in a else ["★원문덤프"]) + ([] if re.search(r"보장되지\s*않", a) else ["★비보장고지_누락"]) + (["★엔티티혼입"] if _entity_mix(a) else []) + ([] if not re.search(r"(?m)^\s*-\s", a.split("\n\n")[0]) else ["머리말없는_항목"]) + ([] if not re.search(r"(?m)^\s*\d+[.)][^\n]*(?:세제|절세)", a) else ["세제블록_잔존"]) + ([] if not re.search(r"[\]\)]\s*(?:\([^)]*\))?\s*(?:이나|과|와)\s*(?:기타\s*)?원리금\s*보장형", a) else ["★유형혼합"]) + ([] if not re.search(r"원리금\s*보장형[^\n.]{0,20}안정적인\s*수익을\s*제공", a) else ["안정수익단정"])),
]


def main():
    ft = open("mini34_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("3문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini34_out.txt (판정 표시는 후보일 뿐, 원문 확인 필요)")
