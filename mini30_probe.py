# -*- coding: utf-8 -*-
"""mini30_probe.py — v13.52 재확인 3문항(T6·T11·M2).
사용: python mini30_probe.py && python claim_check.py mini30_out.txt && python cite_check.py mini30_out.txt"""
...
Q = [
    ("T6", "미래에셋 TDF2045랑 삼성클래식연금 주식형 중에 위험등급이 더 낮은 건 뭐예요?",
     lambda a: ([] if re.search(r"[1-6]\s*등급", a) else ["등급_누락"]) + ([] if "대조 불가" not in a else ["대조불가_고지"]) + ([] if re.search(r"삼성클래식[^\n]{0,40}2\s*등급|2\s*등급[^\n]{0,40}삼성클래식|주식[^\n]{0,30}2\s*등급", a) else ["삼성주식2등급_누락"]) + ([] if "0.47" not in a else ["★창작수치0.47"]) + ([] if not (re.search(r"2\s*등급[^\n]{0,60}보다\s*(?:더\s*)?높", a) and len(re.findall(r"2\s*등급", a.split("핵심 차이")[0])) >= 2) else ["★동일등급_우열단정"]) + ([] if not re.search(r"등급\)\s*/\s*\(", a) else ["★상충셀_잔존"]) + ([] if re.search(r"등급 비교\(근거 문서 기준\)|검증 불가|위험등급이 같으므로", a) or "보다" not in a else ["우열_미확정문장"])),
    ("T11", "30대 직장인인데 공격적으로 굴릴 연금펀드 하나 추천해줘",
     lambda a: ([] if "근거 문서:" in a else ["근거문서줄_없음"]) + ([] if re.search(r"보장되지\s*않", a) else ["★비보장고지_누락"]) + ([] if "우선순위" in a or not re.search(r"(?m)^\s*2[.)]\s", a) else ["무순위고지_누락"]) + ([] if not re.match(r"^\s*-\s", a) else ["★머리말없는_항목"]) + ([] if not re.search(r"상품 예시:\s*TIGER[^\n]*은\(는\)\s*$", a, re.M) else ["★상품예시_꼬리"]) + (["★엔티티혼입"] if _entity_mix(a) else [])),
...
def main():
    ft = open("mini30_out.txt", "w", encoding="utf-8"); bad = 0
    for qid, q, chk in Q:
        t0 = time.time()
        try:
        ...
        ft.write(f"\n{'='*70}\n[{qid}] {q}\n판정: {f or 'OK'}\n--- trace ---\n{tr}\n--- 답변 ---\n{a}\n"); time.sleep(0.5)
    ft.close(); print("=" * 50); print("3문항 모두 OK" if not bad else f"확인 필요 {bad}문항 → mini30_out.txt (판정 표시는 후보일 뿐, 원문 확인 필요)")
